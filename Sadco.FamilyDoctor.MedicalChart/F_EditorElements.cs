...
{
	public partial class F_EditorElements : Form
	{
		...
		private void f_InitCreateControls() {
			var type = typeof(Core.Entities.Controls.I_Control);
		...
		private void f_PopulateTVControls(Cl_GroupsControl a_Group, TreeNodeCollection a_TreeNodes) {
			...
			// Загрузка контролов
			foreach (Cl_CtrlText control in Cl_App.m_DataContext.p_Elms_Text.Where(t => t.p_BaseControl.p_ParentGroupID == a_Group.p_ID)) {
			...
			foreach (Cl_CtrlComboBox control in Cl_App.m_DataContext.p_Elms_ComboBox.Where(t => t.p_BaseControl.p_ParentGroupID == a_Group.p_ID)) {
				Cl_CtrlTreeNode ctrlTreeNode = new Cl_CtrlTreeNode();
				ctrlTreeNode.p_Control = control;
				ctrlTreeNode.f_AddToTreeNode(node.Nodes);
				f_SetMenuImage(control.p_BaseControl.p_MenuIcon, control.p_BaseControl.p_Image);
