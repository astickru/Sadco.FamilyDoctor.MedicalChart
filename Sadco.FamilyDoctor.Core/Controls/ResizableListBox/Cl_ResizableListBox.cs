...
namespace Sadco.FamilyDoctor.Core.Controls.ResizableListBox
{
    ...
    /// </summary>
    public class Cl_ResizableListBox : System.Windows.Forms.Panel
    {
        ...
        private void ItemInserted(int index)
        {
            ...
            m_Items.Info(index).HeightValid = false;
            if (m_Items.Count > index + 1)
                m_Items.Info(index + 1).HeightValid = false;
        }

        public Rectangle GetItemRectangle(int index)
        {
            var rectangle = new Rectangle(0, 0, Width, ItemHeight);
            if (index < Items.Count)
            {
                var scrollPos = this.VerticalScroll.Value;
                int top = -scrollPos;
                for (int i = 0; i < index; i++)
                {
                    var info = m_Items.Info(i);
                    top += info.Height;
                }
                var infoCtrl = m_Items.Info(index);
                rectangle = new Rectangle(0, top, Width, infoCtrl.Height);
            }
            return rectangle;
        }
        ...
        public int IndexFromPoint(Point point)
        {
            ...
            {
                var info = m_Items.Info(i);
                if (y <= point.Y && y + info.Height >= point.Y)
                {
                    return i;
                }
                y += info.Height;
